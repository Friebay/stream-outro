...
def find_most_mentioned_person(chat_file_path):
    ...
    try:
        ...
        sorted_mentions = sorted(
            mentioned_users_count.items(), key=lambda x: (x[1], x[0]), reverse=True
        )
    ...
    try:
        ...
        for username, count in sorted_users:
            if count > 0:  # Only show users who mentioned others
                ...
                print(
                    f"  Mentioned: @{', @'.join(sorted(user_unique_mentions[username]))}"
                )
...
def get_newest_chat_file(chat_file_folder: str):
    ...
    newest_chat_file = max(
        chat_files, key=lambda x: os.path.getmtime(os.path.join(chat_file_folder, x))
    )
...
def outro():
    ...
    chatters = [
        ...
        for user in first_message_timestamps
    ]
    ...
    emote_pattern = re.compile(
        r"\b(" + "|".join(re.escape(emote) for emote in emote_names) + r")\b"
    )
    ...
    with open(emotes_file_path, "r", encoding="utf-8") as emotes_file:
        csv_reader = csv.DictReader(emotes_file)
        for row in csv_reader:
            emote_data[row["Emote"]] = {"name": row["Emote"], "webp": row["Image URL"]}
    ...
    # Compile a regular expression pattern to match emotes
    emote_pattern = re.compile(
        r"\b(" + "|".join(re.escape(emote) for emote in emote_data.keys()) + r")\b"
    )
    ...
    top_mentioner, unique_count, mentioned_users = (
        find_person_with_most_unique_mentions(chat_file_path)
    )
    ...
    return render_template(
        ...
        person_who_typed_the_most_emotes=person_who_typed_the_most_emotes,
        chatters=chatters,
        follower_amount=0,
        person_who_was_mentioned_the_most_times=most_mentioned,
    )
